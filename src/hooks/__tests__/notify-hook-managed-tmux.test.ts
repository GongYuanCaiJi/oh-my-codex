...
describe('notify-hook managed tmux windows fallback', { concurrency: false }, () => {
  ...
  const originalOmxEnv = new Map<string, string | undefined>(
    isolatedOmxEnvNames.map((name) => [name, process.env[name]]),
  );

  beforeEach(() => {
    for (const name of isolatedOmxEnvNames) delete process.env[name];
  });
  ...
  afterEach(() => {
    ...
    for (const name of isolatedOmxEnvNames) {
      const original = originalOmxEnv.get(name);
      if (typeof original === 'string') process.env[name] = original;
  ...
  it('matches canonical tmux ownership tags when invoked by native payload alias', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omx-managed-tmux-canonical-tag-native-alias-'));
    try {
      const stateDir = join(cwd, '.omx', 'state');
      const canonicalSessionId = 'omx-canonical-session';
      const nativeSessionId = 'codex-native-session';
      const tmuxSessionName = 'omx-canonical-tagged-session';
      await mkdir(stateDir, { recursive: true });
      await writeSessionStart(cwd, canonicalSessionId, { tmuxSessionName });
      const current = JSON.parse(await readFile(join(stateDir, 'session.json'), 'utf-8'));
      await writeFile(join(stateDir, 'session.json'), JSON.stringify({
        ...current,
...
if [[ "$cmd" == "show-option" ]]; then
  ...
  if [[ "$pane" == "true" && "$target" == "%42" && "$option" == "@omx_pane_instance_id" ]]; then
    echo "${canonicalSessionId}"
    exit 0
  fi
  if [[ "$pane" == "false" && "$target" == "${tmuxSessionName}" && "$option" == "@omx_instance_id" ]]; then
    echo "${canonicalSessionId}"
    exit 0
  fi
  exit 1
...
`, async () => {
        ...
        const managed = await resolveManagedSessionContext(
          cwd,
          { session_id: nativeSessionId },
          { allowTeamWorker: false, paneTarget: '%42' },
        );
        assert.equal(managed.managed, true);
        assert.equal(managed.invocationSessionId, nativeSessionId);
        assert.equal(managed.canonicalSessionId, canonicalSessionId);
        assert.equal(managed.currentTmuxPaneInstanceId, canonicalSessionId);

        const verified = await verifyManagedPaneTarget('%42', cwd, { session_id: nativeSessionId }, { allowTeamWorker: false });
        assert.equal(verified.ok, true);
        assert.equal(verified.paneInstanceId, canonicalSessionId);
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('does not treat inherited OMX_SESSION_ID as managed-session authority', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omx-managed-tmux-env-not-authority-'));
    const previousOmxSessionId = process.env.OMX_SESSION_ID;
    try {
      const sessionId = 'omx-env-session';
      ...
      const result = await resolveManagedSessionContext(cwd, {}, { allowTeamWorker: false });

      assert.equal(result.managed, false);
      assert.equal(result.reason, 'missing_session_id');
      assert.equal(result.invocationSessionId, '');
    } finally {
