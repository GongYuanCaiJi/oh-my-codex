...
function evidenceFromTeamEvent(event: TeamEvent, planVersion: number, laneTaskMap: Record<string, string>): RuningTeamWorkerEvidence | null {
  ...
  const filesChanged = Array.isArray(metadata.files_changed) ? metadata.files_changed.filter((v): v is string => typeof v === 'string') : [];
  const commands = Array.isArray(metadata.commands) ? metadata.commands.filter((v): v is string => typeof v === 'string') : [];
  const tests = Array.isArray(metadata.tests) ? metadata.tests.filter((v): v is string => typeof v === 'string') : [];
  return {
...
export async function ingestTeamEvidence(cwd: string, sessionId: string): Promise<RuningTeamWorkerEvidence[]> {
  ...
  for (const event of events) {
    ...
    const record = evidenceFromTeamEvent(event, session.plan_version, adapter.lane_task_map);
